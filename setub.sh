...
echo -e "
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright${FONT}     (C)$GRAY NF TUNNELING$NC             │
    └───────────────────────────────────────────────┘
         ${RED}Autoscript Premium vpn lite (multi port)${FONT}    
${RED} Jangan install script ini menggunakan koneksi Vpn ${FONT}"
echo ""
sleep 3
...
if [[ $nama = "" ]]; then
echo " NF TUNNEL" > /etc/xray/username
else
...
print_success "successfully SSHD"
}
clear
...
print_success "successfully Dropbear"
}
clear
function ins_vnstat(){
clear
...
clear
print_install "INSTALLING NOOBZVPNS PREMIUM "
cd
apt install git -y
...
clear
print_install "restart all packages"
